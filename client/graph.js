...
//		this.dataAgent = new DataAgent();
	...
	addNode(obj){
		...
		if(nodeExists!=undefined) // node found
			return nodeExists;
		else{
	...
	/***********************************
	LAY OF THE CODE BELOW:
	1.Render => {
		a.call parseResults and keep results ready
		b 1.instantiate the force layout
		b 2.prepare the svg
		c.attach event handlers (drag,zoom)
		d.call update()
		e.define the internal functions
	}
	2.Update => {
		a.create force layout with results
		b.attach event handlers (click,hover,dblclick)
	...
	***********************************/
		...
		return new Promise((resolve,reject)=>{
			...
			.then(vals => { 
			  ...
			  var svg = d3.select("#svg_area")
						...
						//.call(zoom);
					function zoomed() {
						canvasG.attr("transform", d3.event.transform);
						if(_this.graphComps.textG)  {
							if(d3.event.scale >= _this.scaleToShowLabels )
	...
	/* 		   
			...
			// Per-type markers, as they don't inherit styles.
				...
				function zoomHandler(){				
					...
					xform.setTranslate(d3.event.transform);
	...
	update(){
		...
		//this.simu = d3.forceSimulation()
		//this.simulation.force("links")
		this.sim.force("link",d3.forceLink().id(function(d) { return d.id; }));
		this.sim.force("collision",d3.forceCollide().radius(30));
		
		//console.log(this.sim.nodes());
		//console.log("listing this.links..")
		//console.log(this.links)
		//console.log("listing sim.force('link')")
		//console.log(typeof(this.sim.force("link")))
		...
		var linkData = this.graphComps.circleG
			.style('z-index',3)
			.selectAll("path")
			.data(this.links, d => {return d.rid});
		...
		this.path = linkData.enter().append("path")
			...
			.merge(linkData) //v4 Enter + Update
		 ...
		 this.circle = nodeData.enter().append("circle")
			...
			.attr("class", d => {
				...
				if(d.name == this.rootRid) 
					{
						console.log("root!")
						return "node root-vertex ";
						}
				else return "node branch-vertex "
			})
			.attr("id",d => {
				return d.name.replace('#','r').replace(':','-')
			})
			.merge(nodeData) //v4 Enter + Update
			...
			function fetchLabelInfo(d) {
				//console.log(d);
					...
					.then(val => {
						...
						_this.infoPane.$set(_this.infoPane,'infoPaneText',htmlText);
			...
			function colorEdges(edgeArr, direction){
				...
				if(edgeArr)
				{
					...
					path.classed(classVal,true) //add the class

					if(path._groups[0][0]){

						var linkObj = _this.links[_this.findEdge(_e)] //get the link object from the index of the current edge id
						
						if(direction=='in')//depending on the direction, get the in/out prop and pass to proc
							colorNode(linkObj.source,'source','source-vertex')//call the proc to color the node
						else
							colorNode(linkObj.target,'target','target-vertex')
			...
			function resetNodeColors(){
				resetAttr('source-vertex')
				resetAttr('target-vertex')
			
			}
			function colorNode(node,type,classVal){
				console.log(node)
				var e = node.rid
				var htmlId = "#"+e.replace('#','r').replace(':','-');
				var circle = d3.select(htmlId) , attrW = new AttrWrapper();
				circle.classed(classVal,true) 

				if(circle._groups[0][0]){ //if the element is present in the html
	
				}
				
			}
			function resetAttr(classVal){			
					d3.selectAll("."+classVal).classed(classVal,false);
				}
			...
			function selectVertex(d)  {	// HANDLER FOR SELECTING A VERTEX
						//reset the old selected nodes
						resetAttr('selected-node')			
